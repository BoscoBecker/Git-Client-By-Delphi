unit Git.Classe.Console.Capture;
interface
uses
  System.Threading,
  System.SysUtils;
procedure CaptureConsoleOutputSync(const ACommand: String; CallBack: TProc<string>);  overload;
function CaptureConsoleOutputSync(const ACommand: string): string; overload;
function CaptureConsoleOutputAsync(const ACommand: String; CallBack: TProc<string>):ITask;

implementation
uses
  Winapi.Windows, System.Classes;
function CaptureConsoleOutputSync(const ACommand: string): string;
const
  CReadBuffer = 40000;
var
...
begin
  ...
  if not CreatePipe(hRead, hWrite, @saSecurity, 0) then Exit;
  try
    ...
    if not Createprocess(nil, PChar(ACommand+''), @saSecurity, @saSecurity, true, NORMAL_PRIORITY_CLASS, nil, nil, suiStartup, piProcess) then Exit;
    try
      var txt := '';
      repeat
